...
open class XMLEncoder {
    ...
    var options: XMLEncoderOptions {
        return XMLEncoderOptions(dateEncodingStrategy: dateEncodingStrategy,
    ...
    public init(options: XMLEncoderOptions) {
        ...
        dataEncodingStrategy = options.dataEncodingStrategy
        nonConformingFloatEncodingStrategy = options.nonConformingFloatEncodingStrategy
        keyEncodingStrategy = options.keyEncodingStrategy
        nodeEncodingStrategy = options.nodeEncodingStrategy
        stringEncodingStrategy = options.stringEncodingStrategy
    ...
    open func encode<T: Encodable>(_ value: T) throws -> Data {
        ...
        encoder.nodeEncodings.append(nodeEncodingStrategy.nodeEncodings(forType: T.self, with: encoder))

        let topLevel = try encoder.addToXMLContainer(value)
        let attributes = rootAttributes?.map(XMLAttributeRepresentable.init) ?? []

        let elementOrNone: XMLElementRepresentable?

        let rootKeyString = rootKey ?? "\(T.self)".convert(for: keyEncodingStrategy)
