...
class SymbolProviderTest {
    ...
    fun `it adds namespace to nested structure`() {
        ...
        var model = Model.assembler()
            ...
            .assemble()
            .unwrap()
        model = NestedShapeTransformer.transform(model, service)
        val provider: SymbolProvider = SwiftCodegenPlugin.createSymbolProvider(model, model.defaultSettings())
        val structSymbol = provider.toSymbol(struct1)
        assertEquals("Struct1", structSymbol.name)
        assertEquals(structSymbol.namespace, "QuuxClientTypes")
    }

    @Test
    fun `it does not add namespace to error structure`() {
        val memberQuux = MemberShape.builder().id("foo.bar#ErrorStruct\$quux").target("smithy.api#String").build()
        ...
        val model = Model.assembler()
            .addShapes(struct1, memberQuux)
            .assemble()
            .unwrap()

        val provider: SymbolProvider = SwiftCodegenPlugin.createSymbolProvider(model, model.defaultSettings())
        val structSymbol = provider.toSymbol(struct1)
        assertEquals("ErrorStruct", structSymbol.name)
        assertEquals(structSymbol.namespace, "")
    }
}
