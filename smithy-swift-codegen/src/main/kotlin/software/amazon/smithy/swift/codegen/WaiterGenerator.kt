    ...
    override fun writeAdditionalFiles(ctx: CodegenContext, protoCtx: ProtocolGenerator.GenerationContext, delegator: SwiftDelegator) {
        ...
        delegator.useFileWriter("${ctx.settings.moduleName}/Waiters.swift") { writer ->
            ...
            waitedOperations.forEach { waitedOperation ->
                val waiterInfo = paginatedIndex.getPaginationInfo(service, paginatedOperation).getOrNull()
                ...
                renderWaiterForOperation(writer, ctx, service, paginatedOperation, paginationInfo, paginationItemInfo)
            }
        }
    }
