...
class IdempotencyTokenMiddlewareGenerator(
     ...
     * ```
     * operationStack.initializeStep.intercept(position: .before, id: "IdempotencyTokenMiddleware") { (context, input, next) -> Result<SdkHttpRequestBuilder, Error> in
    ...
    fun renderIdempotencyMiddleware() {
        writer.openBlock("$operationMiddlewareStackName.initializeStep.intercept(position: .before, id: \"IdempotencyTokenMiddleware\") { (context, input, next) -> Result<SdkHttpRequestBuilder, Error> in", "}") {
