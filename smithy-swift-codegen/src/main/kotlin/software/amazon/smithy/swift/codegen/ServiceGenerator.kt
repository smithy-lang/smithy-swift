...
class ServiceGenerator(
    ...
    private val serviceConfig: DefaultConfig by lazy {
        DefaultConfig(writer, serviceSymbol.name)
    }
    ...
    private fun renderSwiftProtocol() {
        ...
        writer.declareSection(ConfigurationProtocol, sectionContext) {
            writer.openBlock("public protocol \$L : \$L {", "}", serviceConfig.typeProtocol, serviceConfig.getTypeInheritance()) {
                writer.write("var endpoint: \$T { get }", SwiftTypes.String)
            }
        }.write("")
    }

    object ConfigurationProtocol : SectionId
