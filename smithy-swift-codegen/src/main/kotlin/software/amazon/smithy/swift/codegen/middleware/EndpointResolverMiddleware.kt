...
class EndpointResolverMiddleware(
    ...
    override fun render(writer: SwiftWriter, op: OperationShape, operationStackName: String) {
        ...
        writer.write("$operationStackName.${middlewareStep.stringValue()}.intercept(position: ${position.stringValue()}, middleware: \$N<\$N, \$N>(${middlewareParamsString()}))",
            ClientRuntimeTypes.Core.EndpointResolverMiddleware, output, outputError)
    }

    private fun middlewareParamsString(): String {
        return "endpointResolver: DefaultEndpointResolver(config: config)"
    }
}