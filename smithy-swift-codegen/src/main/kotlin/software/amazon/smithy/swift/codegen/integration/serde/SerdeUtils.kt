...
class SerdeUtils {
    companion object {
        fun useSchemaBased(ctx: ProtocolGenerator.GenerationContext) = useSchemaBased(ctx.settings, ctx.service)

        fun useSchemaBased(
            settings: SwiftSettings,
            model: Model,
        ) = useSchemaBased(settings, model.expectShape<ServiceShape>(settings.service))

        private fun useSchemaBased(
            settings: SwiftSettings,
