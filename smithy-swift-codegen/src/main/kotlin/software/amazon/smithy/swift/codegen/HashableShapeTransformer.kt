...
object HashableShapeTransformer {
    ...
    private fun transformInner(model: Model) : Model? {
        ...
        val shapesNeedingHashable = model.operationShapes.filter { isHashable(model, it) && it.hasTrait<HashableTrait>() }
        ...
        shapesNeedingHashable.forEach { shape ->
    ...
    private fun isHashable(model: Model, shape: Shape): Boolean {
        return model.listShapes.any { it.member.target == shape.toShapeId() } || model.setShapes.any { it.member.target == shape.toShapeId() }
    }
