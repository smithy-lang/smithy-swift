...
class HttpRestJsonProtocolGenerator : HttpBindingProtocolGenerator() {
            ...
            with(writer) {
                ...
                openBlock("extension ${ctx.symbolProvider.toSymbol(ctx.service).name}Types {", "}") {
                    openBlock(
                        ...
                        ClientRuntimeTypes.RestJSON.RestJSONError,
                        SwiftTypes.Error
                    ) {
                    ...
                    openBlock(
                        ...
                        openBlock("switch restJSONError.errorType {", "}") {
                            ...
                            errorShapes.forEach { errorShape ->
                                ...
                                write(
                                    "case \$S: return try await \$L(httpResponse: httpResponse, decoder: decoder, message: restJSONError.errorMessage)",
                                    errorShapeName,
                                    errorShapeType
                                )
                            }
                            write(
                                "default: return try await \$N.makeError(httpResponse: httpResponse, message: restJSONError.errorMessage, typeName: restJSONError.errorType)",
                                unknownServiceErrorSymbol
                            )
                        }
                    }
