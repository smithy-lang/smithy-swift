...
class EndpointResolverGenerator(
    ...
    private fun renderResolverProtocol(writer: SwiftWriter) {
        writer.openBlock("public protocol \$L {", "}", EndpointSymbols.EndpointResolver) {
    ...
    private fun renderResolver(writer: SwiftWriter, endpointRules: EndpointRuleSet?) {
        writer.openBlock("public struct \$L: \$L  {", "}", EndpointSymbols.DefaultEndpointResolver, EndpointSymbols.EndpointResolver) {
            writer.write("")
            endpointRules?.let {
            ...
            writer.openBlock("public init() throws {", "}") {
                endpointRules?.let {
                    writer.write("engine = try \$L(partitions: \$S, ruleSet: ruleSet)", ClientRuntimeTypes.Core.EndpointsRuleEngine, partitionDefinition)
                }
            }
            writer.write("")
            writer.openBlock(
