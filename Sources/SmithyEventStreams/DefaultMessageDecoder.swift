...
public class DefaultMessageDecoder: MessageDecoder {
    ...
    private let logger = SwiftLogger(label: "DefaultMessageDecoder")
    ...
    public init() {
        self.decoder = EventStreamMessageDecoder(
            ...
            onError: { code, message in
                ...
                self.error = EventStreamError.decoding("\(code): \(message)")
            }
        )
    }
    ...
    public func endOfStream() throws {
        ...
        guard decodedPayload.isEmpty && decodededHeaders.isEmpty else {
            throw EventStreamError.decoding("Stream ended before message was complete")
        }
    }
