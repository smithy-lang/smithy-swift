...
class WaiterConfigurationTests: XCTestCase {
    ...
    func test_init_minDelayDefaultsTo2WhenNotGiven() async throws {
        let subject = try WaiterConfiguration(minDelay: nil, maxDelay: nil, acceptors: acceptors)
    ...
    func test_init_throwsWhenAcceptorsDoesNotContainSuccessState() async throws {
        ...
        XCTAssertThrowsError(try WaiterConfiguration(minDelay: nil, maxDelay: nil, acceptors: badAcceptors)) { error in
    ...
    func test_init_throwsWhenAcceptorsIsEmpty() async throws {
        ...
        XCTAssertThrowsError(try WaiterConfiguration(minDelay: nil, maxDelay: nil, acceptors: noAcceptors)) { error in
            XCTAssert(error is WaiterConfigurationError)
        }
    }
}
